...
Test 'Notify failure' for existing topic
    ...
    Then No Process Instance available at topic
    ...    topic=${existing_topic}
    ...    error_message=Notifying Failure failed. Process instance should not be available anymore at service task.
...
Test 'Notify failure' with unfinished retries
    ...
    Then Process Instance available at topic
    ...    topic=${existing_topic}
    ...    error_message=Notifying Failure failed. Process instance should not be available anymore at service task.
...
process instance fetched
    ...
    Should Not be Empty
    ...    ${process_instance}
    ...    Failure while setting up test case. No process instance available to send failure for.
...
Process instance is incident
    ...
    ${process_instance_after_failure}    Get process instances
    ...    process_instance_ids=${process_instance}[process_instance_id]
    log    ${process_instance_after_failure}
    Should Not Be Empty
    ...    ${process_instance_after_failure}
    ...    Notifying Failure failed. Process instance is instance but should be an incident.

    ${incident}    Get incidents    process_instance_id=${process_instance}[process_instance_id]
    log    ${incident}
    Should Not be Empty
    ...    ${incident}
    ...    Getting incident failed. There is no incident availabe matching the process instance.
